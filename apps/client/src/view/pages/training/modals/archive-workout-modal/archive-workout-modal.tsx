...
export function ArchiveWorkoutModal(props: ArchiveWorkoutModalProps) {
	const { isOpen, visibility, onArchiveWorkout, onClose } = props;
	return (
			...
			<Modal.Header>
				...
				<Modal.Description>
					{`Tem certeza que deseja ${
						visibility ? "arquivar" : "desarquivar"
					} esse treino?`}
				</Modal.Description>
				<small className="text-md mt-4 mb-4">
					{`Este treino ${
						visibility && "não"
					} poderá ser visualizado pelo atleta!`}
				</small>
				<Modal.Footer>
					<Button onClick={onClose} variant={"destructive"}>
						Cancelar
					</Button>
					<Button onClick={onArchiveWorkout}>
						{visibility ? "Arquivar" : "Desarquivar"}
					</Button>
				</Modal.Footer>
			</Modal.Header>
